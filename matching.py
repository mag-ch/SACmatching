...
from compareLocations import get_clusters
...
def edge(row_1, row_2):
    if (row_1[1] == row_2[1]):
        return False
    # puts edge if there's an overlap on 
    if (type(row_1[5]) == float or type(row_2[5]) == float):
        return True if (row_1[5] == float and row_2[5] == float) else False
    if ((type(row_1[6]) == float and type(row_2[6]) == float)):
        return True
    if (type(row_1[6]) == float): 
        teach_1 = set(row_1[5].split(','))
        learn_2 = set(row_2[6].split(','))
        return len(teach_1.intersection(learn_2)) > 0
    if (type(row_2[6]) == float):
        teach_2 = set(row_2[5].split(','))
        learn_1 = set(row_1[6].split(','))
        return len(teach_2.intersection(learn_1)) > 0 
    if ((row_1[4] != "No" or row_2[4] != "No") and row_1[11] != row_2[11]):
        return False
    teach_1 = set(row_1[5].split(','))
    teach_2 = set(row_2[5].split(','))
    learn_1 = set(row_1[6].split(','))
    learn_2 = set(row_2[6].split(','))
    return len(teach_1.intersection(learn_2)) > 0 and len(teach_2.intersection(learn_1)) > 0 
def generate_graph():
    """
    ...
    for i, row_1 in data.iterrows():
        for indx, row_2 in data.iterrows():
            if edge(row_1, row_2):
                user_graph[i].add(indx)
                user_graph[indx].add(i)
    return user_graph
def get_info(matching):
    data = get_clusters()
    matching_df = pd.DataFrame(index=range(len(data)), columns=['email', 'name', 'matched_email', 'matched_name', 'familiar_cuisine', 'learn_cuisine', 'matched_familiar_cuisine', 'matched_learn_cuisine', 'loc_pref', 'cluster_id', 'matched_cluster_id'])
    for k in matching:
        v = matching[k]
        matching_df.loc[k]['email'] = data.iloc[k][1] 
        matching_df.loc[k]['name'] = data.iloc[k][2]
        matching_df.loc[k]['familiar_cuisine'] = data.iloc[k][5]
        matching_df.loc[k]['learn_cuisine'] = data.iloc[k][6]
        matching_df.loc[k]['matched_email'] = data.iloc[v][1]  
        matching_df.loc[k]['matched_name']= data.iloc[v][2]
        matching_df.loc[k]['matched_familiar_cuisine'] = data.iloc[v][5]
        matching_df.loc[k]['matched_learn_cuisine'] = data.iloc[v][6]
        matching_df.loc[k]['loc_pref'] = "yes" if (data.iloc[v][4] != "No" or data.iloc[k][4] != "No") else "no"
        matching_df.loc[k]['cluster_id'] = data.iloc[k][3]
        matching_df.loc[k]['matched_cluster_id'] = data.iloc[v][3]
    print(matching_df)
     
    matching_df.to_csv('out.csv', index=False)
        



user_graph = generate_graph()

get_info(matching(user_graph))
